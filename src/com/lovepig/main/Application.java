...
public class Application extends BaseActivity implements ServiceConnection, OnClickListener {
    ...
    public void enterNews() {
        Application.this.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        LogInfo.LogOut("BaseActivity", "enter...........onKeyUp");

        if (keyCode == KeyEvent.KEYCODE_BACK) {
            if (isBack) {// 解决有输入发弹出时，按返回键，界面返回到上一页的情况
                isBack = false;
            } else {
                return true;
            }
             if (currentManager instanceof OnlineNewsManager) {
                 currentManager.sendEmptyMessage(OnlineNewsManager.STATE_DETAILSBACK);
                 return true;
            }
