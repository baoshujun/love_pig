...
public class UserManager extends BaseManager {
    ...
    public static final int STATE_TOBINDHTTP = 6;// 去绑定
    public static final int STATE_TOBINDSUCESS = 7;// 绑定成功
    public static final int STATE_TOBINDFAIL = 8;// 绑定失败
    public static final int STATE_REGISTERSUCESS = 9;// 注册成功
    public static final int STATE_REGISTERFAIL = 10;// 注册失败
    public static final int STATE_UNBINDSUCESS = 11;// 解除绑定成功
    public static final int STATE_UNBINDFAIL = 12;// 解除绑定失败
    ...
    public static final int STATE_NAME_CAN_USE = 18;// 用户文号可用
    public static final int STATE_NAME_USEED = 19;// 用户文号不可用
...
//            showToast("绑定成功");
            break;
        case STATE_UNBINDSUCESS:// 解绑成功
//            mUserUnBindDC.setUserID();
//            dcEngine.setMainDC(registerDC);
//            dismissLoading();
            break;
        case STATE_UNBINDFAIL:// 解绑失败
            if (msg.obj != null) {
                showToast((String) msg.obj);
            } else {
    ...
    public void Back() {
//        if (getNowShownDC() == bindDC) {
//            dcEngine.setMainDC(registerDC);
//        } else if (getNowShownDC() == mUpdateUserInfoDC || getNowShownDC() == mUserModiyPWDC) {
//            dcEngine.setMainDC(mainDC);
//        } else {
//            back();
//        }
...
//            enterSubDC(bindDC);
        ...
        case R.id.register_register_reset_btn:// 重置
            registerDC.Reset();
            break;
        case R.id.register_userid_checked_btn:// 检测文号
            if (registerDC != null) {
        ...
        case R.id.account_modifyinfo:// 修改个人信息
            if (!Configs.isCheckin || Configs.userid == null) {
                dcEngine.setMainDC(registerDC);
            } else {
                if (Configs.isChangededPWD == 0) {
                    showAlert("请先修改密码，再修改用户信息！", new TlcyDialogListener() {

                        @Override
                        public void onClick() {
                            onClicked(R.id.account_modifypwd);
                        }
                    }, null);
                } else {
        ...
        case R.id.account_modifypwd:// 修改用户密码
            ...
            break;
        case R.id.account_unbind:// 取消绑定
            if (!Configs.isCheckin || Configs.userid == null) {
                dcEngine.setMainDC(registerDC);
            } else {
                if (Configs.isChangededPWD == 0) {
                    showAlert("请先修改密码，才能解除绑定！", new TlcyDialogListener() {
...
//                    mUserUnBindDC.setUserID();
//                    enterSubDC(mUserUnBindDC);
                }

            }
            break;
        case R.id.binduser_ok_btn:// 确定绑定
            ToBinding();
            break;
        case R.id.userunbind_ok_btn:
//            if (mUserUnBindDC.getUserPWD() == null) {
//                return;
//            }
            showAlert("解除绑定将删除所有数据,您确认解除吗？", new TlcyDialogListener() {
...
//        }
    ...
    private void initDC() {
        ...
        if (registerDC == null) {
            registerDC = new UserRegisterView(context, R.layout.user_register, this);
        }
//        if (bindDC == null) {
//            bindDC = new UserBindDC(context, R.layout.user_bind, this);
//        }
//        if (mUserUnBindDC == null) {
//            mUserUnBindDC = new UserUnBindDC(context, R.layout.user_unbind, this);
//        }
        if (mUserModiyPWDC == null) {
            mUserModiyPWDC = new UserModiyPWView(context, R.layout.user_modifypwd, this);
        }
        if (mUpdateUserInfoDC == null) {
            mUpdateUserInfoDC = new UserUpdateUserInfoView(context, R.layout.user_updateuserinfo, this);
        }
    }
    ...
    public void onLoadingCacel() {
        ...
        engine.stopToBiinding();
        engine.stopUnbindTask();
    ...
    public void EnterUserManager(int backtxtid) {
        if (!Configs.isCheckin || Configs.userid == null || !isGetUserInfo) {
